...
const form = useForm({
  batchassign_id: props.dailyOperation?.batchassign_id || '',
  ...
  culling_note: props.dailyOperation?.cullings?.[0]?.note || '',
  ...
  vaccine_schedule_detail_id: props.dailyOperation?.vaccines?.[0]?.vaccine_schedule_detail_id || '',
  vaccine_id: props.dailyOperation?.vaccines?.[0]?.vaccine_id || '',
...
function submit() {
  ...
  form.put(route('daily-operation.update', props.dailyOperation.id), {
    onSuccess: () => {
      // Redirect to the list page after successful update
      window.location.href = `/daily-operation/stage/${props.stage}`;
    },
  })
}
</script>
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    <Head :title="`Daily Operation - ${currentTitle} - Edit`" />
