...
const exportPdf = (orientation: 'portrait' | 'landscape' = 'portrait') => {
    const url = route('reports.shed.pdf', { ...filters.value, orientation });
    window.open(url, '_blank');
};
const exportExcel = () => {
    const url = route('reports.shed.excel', { ...filters.value });
    window.open(url, '_blank');
};
...
const submit = () => {
    ...
    if (editingShed.value) {
        form.put(route('shed.update', editingShed.value.id), {
            ...
            onError: () => Swal.fire('Error!', 'Could not update shed.', 'error'),
        });
    } else {
        form.post(route('shed.store'), {
            ...
            onError: () => Swal.fire('Error!', 'Could not create shed.', 'error'),
        });
    }
};
        ...
        <div class="px-4 py-6">
            <div class="mb-4 flex items-center justify-between">
                ...
                <div class="relative flex items-center gap-2">
                    <!-- Add New Button -->
                    <Button v-if="can('shed.create')" class="bg-chicken text-white hover:bg-yellow-600" @click="openModal()">+ Add New</Button>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-white">
                        <tr v-for="(shed, index) in sheds" :key="shed.id">
                            ...
                            <td class="relative px-6 py-4">
                                ...
                                >
                                    <button class="w-full px-4 py-2 text-left hover:bg-gray-100" @click="openModal(shed)">✏ Edit</button>
                                    <button class="w-full px-4 py-2 text-left hover:bg-gray-100" @click="toggleStatus(shed)">
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex justify-center bg-black/30 pt-6" @click.self="resetForm">
            <div ref="modalRef" class="w-full max-w-2xl rounded-lg border border-gray-300 bg-white shadow-lg" style="top: 100px; position: absolute">
                <div class="flex cursor-move items-center justify-between border-b border-gray-200 p-4" @mousedown="startDrag">
                    <h3 class="text-xl font-semibold text-gray-900">{{ editingShed ? 'Edit Shed' : 'Add New Shed' }}</h3>
                    <button
                        type="button"
                ...
                <div class="flex justify-end border-t border-gray-200 p-4">
                    ...
                    <Button class="bg-chicken text-white" @click="submit">{{ editingShed ? 'Update' : 'Save' }}</Button>
                </div>
            </div>
        </div>
    </AppLayout>
