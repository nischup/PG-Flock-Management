...
const submit = () => {
  ...
  if (editingVaccineType.value) {
    form.put(route('vaccine-type.update', editingVaccineType.value.id), {
      preserveScroll: true,
      onSuccess: () => {
        const i = vaccineTypes.value.findIndex(v => v.id === editingVaccineType.value!.id)
        if (i !== -1) {
          vaccineTypes.value[i] = { ...vaccineTypes.value[i], name: form.name, status: form.status }
        }
        resetForm()
      },
    })
    ...
    form.post(route('vaccine-type.store'), {
      preserveScroll: true,
      onSuccess: (page) => {
        if ((page as any).props?.vaccineTypes) {
          vaccineTypes.value = (page as any).props.vaccineTypes
        } else {
          vaccineTypes.value.unshift({
            id: Date.now(),
            name: form.name,
            status: form.status,
...
<template>
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <!-- Filters -->
    <FilterControls :filters="props.filters" routeName="/vaccine-type" />

    <div class="px-4 py-6">
      <!-- Header -->
      <div class="flex items-center justify-between mb-4">
        <HeadingSmall title="Vaccine Types List" />
        <Button v-if="can('vaccine-type.create')" class="bg-chicken hover:bg-yellow-600 text-white" @click="openModal()">
          + Add New
        </Button>
      </div>
      ...
      <table class="w-full border">
        ...
        <tbody>
          <tr v-for="(v, index) in vaccineTypes" :key="v.id">
            ...
            <td class="p-2 border">
              <span :class="v.status === 1 ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold'">
                {{ v.status === 1 ? 'Active' : 'Inactive' }}
              </span>
            </td>
            <td class="p-2 border">{{ v.created_at }}</td>
            <td class="p-2 border relative">
              ...
              >
                <button class="w-full text-left px-4 py-2 hover:bg-gray-100" @click="openModal(v)">✏ Edit</button>
                <button class="w-full text-left px-4 py-2 hover:bg-gray-100" @click="toggleStatus(v)">
      ...
      >
        ...
        >
          <h3 class="text-xl font-semibold text-gray-900">
            {{ editingVaccineType ? 'Edit Vaccine Type' : 'Add New Vaccine Type' }}
          </h3>
          <button
            type="button"
