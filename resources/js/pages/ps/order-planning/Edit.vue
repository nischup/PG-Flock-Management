...
const props = defineProps<{
  plan: {
    ...
    order_to: string;
    cc: string | null;
    items: Array<{ id: number; order_volume: string; shipping_date: string; supply_base: string }>;
  };
}>();
...
const form = useForm({
  ...
  cc: props.plan.cc ?? '',
  items: props.plan.items.map(item => ({
    id: item.id,
...
<template>
  <AppLayout title="Edit Order Planning">
    <h1 class="text-xl font-bold mb-4">Edit Order Planning</h1>

    <form @submit.prevent="form.put(route('order-plans.update', props.plan.id))" class="space-y-4">
      <div>
