...
// Initialize form with existing classification data
onMounted(() => {
  if (props.classification) {
    // Set basic form data
    form.batchassign_id = props.classification.batchassign_id
    form.operation_date = props.classification.classification_date
    form.total_egg = props.classification.total_eggs

    // Initialize rejected eggs data
    if (props.classification.rejected_eggs) {
      props.classification.rejected_eggs.forEach((rejected: any) => {
        const eggTypeName = rejected.egg_type?.name
        if (eggTypeName && form.hasOwnProperty(eggTypeName)) {
          form[eggTypeName] = rejected.quantity || 0
          form[eggTypeName + '_note'] = rejected.note || ''
        }
      })
    }

    // Initialize technical eggs data
    if (props.classification.technical_eggs) {
      props.classification.technical_eggs.forEach((technical: any) => {
        const eggTypeName = technical.egg_type?.name
        if (eggTypeName && form.hasOwnProperty(eggTypeName)) {
          form[eggTypeName] = technical.quantity || 0
          form[eggTypeName + '_note'] = technical.note || ''
        }
      })
    }
  }
...
function submit() {
  ...
  form.put(route('egg-classification.update', props.classification.id), {
    onSuccess: () => showInfo("Egg classification updated successfully")
  })
}
